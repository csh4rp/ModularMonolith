...
using ModularMonolith.Shared.DataAccess.Mongo.Outbox.Factories;
...
public abstract class CrudRepository<TAggregate, TId> where TAggregate : AggregateRoot<TId> where TId : IEquatable<TId>
{
    ...
    protected CrudRepository(IOptions<AuditLogOptions> auditOptions,
        IOptions<OutboxOptions> outboxOutboxOptions,
        IMongoDatabase database,
        IAuditMetaDataProvider auditMetaDataProvider,
        TimeProvider timeProvider, IOutboxMessageFactory outboxMessageFactory)
    {
    ...
    protected IOptions<AuditLogOptions> AuditOptions { get; }
    ...
    private List<OutboxMessage> CreateOutboxMessages(IEnumerable<DomainEvent> events) => 
        events.Select(_outboxMessageFactory.Create).ToList();
    ...
    private AuditLogEntity CreateDeletedAuditLog(TAggregate aggregate, BsonMemberMap idMemberMap) =>
        new()
        {
            ...
            MetaData = AuditMetaDataProvider.GetMetaData(),
        };
    ...
    private BsonDocument CreateDocument(TAggregate aggregate)
    {
        ...
        var metaData = AuditMetaDataProvider.GetMetaData();
